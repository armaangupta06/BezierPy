...
# For standalone deployment, the root is one level up from the current file
...
try:
    # Import the FastAPI app - adjust import path for standalone deployment
    sys.path.insert(0, root_path)
    from app import app  # Direct import since we're in the API directory
    from routes import path_routes, trajectory_routes  # Direct import
    logger.info("Successfully imported FastAPI app and routes")
    
    # Create a handler for AWS Lambda / Vercel
    # No need to strip /api prefix in standalone deployment
    def log_event(event):
