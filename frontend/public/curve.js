...
function generatePoints(path) {
   ...
   */
  function calculateTrajectory(path, v0, v1, max_v, a_accel, a_decel, max_j, max_w, trap = true) {
    const steps = [];
    let arcLength = 0.0;
   ...
   */
  function pathWithPoints(points, initial_heading, final_heading = null, v = 100, a = 10, tangent_magnitude = 0.5) {
    const path = [];
   ...
   */
  function pathWithPoses(poses, v = 100, a = 10, tangent_magnitude = 0.5) {
    const path = [];
    ...
    if (curr === 0) {
      if (path.length === 2) {
        ...
        acc0 = path[curr].multiply(-6)
               ...
               .add(v1.multiply(-2))
               .add(path[curr + 1].multiply(6));
        acc1 = alpha * (path[curr].multiply(6)
               .add(v0.multiply(2))
               .add(v1.multiply(4))
               .add(path[curr + 1].multiply(-6)))
            + beta * (path[curr + 1].multiply(-6)
               .add(v1.multiply(-4))
               .add(vD.multiply(-2))
